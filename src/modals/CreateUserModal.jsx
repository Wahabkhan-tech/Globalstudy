...
const CreateUserModal = ({ onClose, onSubmit, mode = 'create', user = null, pageRole = '' }) => {
  const [formData, setFormData] = useState({
    id: user?.id || '',
    name: user?.name || '',
    role: user?.role || pageRole || 'student',
    course: user?.course || '',
  ...
  const getHeading = () => {
    const roleText = pageRole === 'student' ? 'Student' : pageRole === 'counselor' ? 'Counselor' : pageRole === 'media_channel' ? 'Media Channel' : 'User';
    if (mode === 'view') return `View ${roleText} Details`;
    return mode === 'create' ? `Create New ${roleText}` : `Edit ${roleText}`;
  };

  const getButtonText = () => {
    if (mode === 'view') return 'Close';
    const action = mode === 'create' ? 'Create' : 'Update';
    return pageRole === 'student' ? `${action} Student` : pageRole === 'counselor' ? `${action} Counselor` : pageRole === 'media_channel' ? `${action} Media Channel` : action;
  };
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" style={{ background: 'rgba(0,0,0,0.5)' }}>
      <div ref={modalRef} className="bg-white rounded-lg p-6 w-full max-w-[90vw] sm:max-w-2xl overflow-y-auto max-h-[90vh]">
        <h2 className="text-xl font-semibold text-gray-800 mb-6">{getHeading()}</h2>
        {mode === 'view' ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-1">ID</label>
              <p className="w-full p-2 text-sm text-gray-900">{formData.id || 'N/A'}</p>
            </div>
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-1">Name</label>
              <p className="w-full p-2 text-sm text-gray-900">{formData.name || 'N/A'}</p>
            </div>
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-1">Role</label>
              <p className="w-full p-2 text-sm text-gray-900">{formData.role.charAt(0).toUpperCase() + formData.role.slice(1) || 'N/A'}</p>
            </div>
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-1">Status</label>
              <p className="w-full p-2 text-sm text-gray-900">{formData.status || 'N/A'}</p>
            </div>
            {formData.role === 'student' && (
              <>
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-700 mb-1">Course</label>
                  <p className="w-full p-2 text-sm text-gray-900">{formData.course || 'N/A'}</p>
                </div>
                <div className="mb-4">
                  <label className="block text-sm font-medium text-gray-700 mb-1">Admission Stage</label>
                  <p className="w-full p-2 text-sm text-gray-900">{formData.admission_stage || 'N/A'}</p>
                </div>
              </>
            )}
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-1">Assigned To</label>
              <p className="w-full p-2 text-sm text-gray-900">{formData.assignedTo || 'Not Assigned'}</p>
            </div>
            <div className="mb-4">
              <label className="block text-sm font-medium text-gray-700 mb-1">Region</label>
              <p className="w-full p-2 text-sm text-gray-900">{formData.region || 'N/A'}</p>
            </div>
          </div>
        ) : (
        ...
        <div className="flex justify-end space-x-2 mt-6">
          ...
          {mode !== 'view' && (
            <button
              type="submit"
              form={mode !== 'view' ? 'create-user-form' : undefined}
